// ----------------------------------------------------------------------------
// HyperConv hybrid - tb_selftest_hybrid.v
// Simulation check for the hybrid board self-test wrapper: clocks
// selftest_top_hybrid with the sobel_x vectors and confirms led_pass
// asserts (and led_fail does not). Proves the on-chip 2-px compare / LED
// logic before it ever reaches a board.
//
//   +IMG/+KER/+EXP override the ROM files (defaults set via -generic_top).
// ----------------------------------------------------------------------------
`timescale 1ns / 1ps

module tb_selftest_hybrid;
