// ----------------------------------------------------------------------------
// HyperConv hybrid - tb_conv_top_hybrid.v
// Self-checking testbench for the 2-pixel/cycle accelerator. Loads the SAME
// golden-model testcases as the baseline (img/kernel/expected hex from
// golden/gen_tests.py), streams the image as 2 pixels/cycle (px0 = even
// column, px1 = odd column, optionally with random bubbles), reassembles the
// interleaved output stream (out0 then out1 per beat) and compares every
// output against the golden model bit-for-bit.
//
// Plusargs:
//   +IMG=<file> +KER=<file> +EXP=<file>   stimulus / golden files
//   +OUT=<file>                           dump DUT outputs (s16 hex, same
//                                         order as baseline dut_out.hex)
//   +GAPS                                 insert random px_valid bubbles
//   +VCD                                  dump dump.vcd
// Parameters N / IMG_W / IMG_H / KSEL / RELU set per-test via -generic_top.
// ----------------------------------------------------------------------------
`timescale 1ns / 1ps

module tb_conv_top_hybrid;
    ...
    localparam NPAIR  = NPIX / 2;      // IMG_W even (RTL constraint)
    ...
    reg                     px_valid = 1'b0;
    reg  [PIX_W-1:0]        px0 = {PIX_W{1'b0}};
    reg  [PIX_W-1:0]        px1 = {PIX_W{1'b0}};
    wire                    out_valid;
    ...
    integer done_seen = 0;
    integer beats = 0;
    integer max_back_to_back = 0;   // consecutive full-rate beats (perf check)
    ...
    always @(posedge clk) begin
        ...
        if (out_valid) begin
            ...
            beats <= beats + 1;
            // out0 = even index, out1 = odd index (interleaved reassembly)
            if (out_cnt < NOUT) begin
                dut_mem[out_cnt] <= out0;
                if (out0 !== $signed(exp_mem[out_cnt])) begin
                    err_cnt <= err_cnt + 1;
                    if (err_cnt < 10)
                        $display("MISMATCH out[%0d] (row %0d col %0d): dut=%0d exp=%0d",
                                 out_cnt, out_cnt/(IMG_W-N+1), out_cnt%(IMG_W-N+1),
                                 out0, $signed(exp_mem[out_cnt]));
                end
            end else begin
                err_cnt <= err_cnt + 1;
                if (err_cnt < 10)
                    $display("SPURIOUS extra output #%0d = %0d", out_cnt, out0);
            end
            if (out_cnt + 1 < NOUT) begin
                dut_mem[out_cnt+1] <= out1;
                if (out1 !== $signed(exp_mem[out_cnt+1])) begin
                    err_cnt <= err_cnt + 1;
                    if (err_cnt < 10)
                        $display("MISMATCH out[%0d] (row %0d col %0d): dut=%0d exp=%0d",
                                 out_cnt+1, (out_cnt+1)/(IMG_W-N+1), (out_cnt+1)%(IMG_W-N+1),
                                 out1, $signed(exp_mem[out_cnt+1]));
                end
            end
            out_cnt <= out_cnt + 2;
        end
    ...
    initial begin
        ...
        $display("TB: outputs=%0d/%0d beats=%0d mismatches=%0d frame_done_pulses=%0d",
                 out_cnt, NOUT, beats, err_cnt, done_seen);
        $display("TB: latency first-pair -> first-output-pair = %0d cycles",
