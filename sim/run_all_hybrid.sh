#!/bin/bin/bash
# HyperConv hybrid - run all golden-model testcases through the 2-px/cycle DUT.
# Usage: sim/run_all_hybrid.sh [test_name ...]   (no args = all tests)
set -uo pipefail
...
SIM="$ROOT/sim"
LOGS="$SIM/logs_hybrid"

cd "$SIM"
rm -rf xsim.dir "$LOGS"
mkdir -p "$LOGS"

echo "== xvlog: compiling RTL + hybrid TB =="
"$VIV/xvlog" "$ROOT"/rtl/*.v "$ROOT"/tb/tb_conv_top_hybrid.v > "$LOGS/xvlog.log" 2>&1 || {
...
for name in "${TESTS[@]}"; do
    ...
    "$VIV/xelab" tb_conv_top_hybrid -s "$snap" --incr \
    ...
    "$VIV/xsim" "$snap" -R \
        ...
        -testplusarg "EXP=$d/expected.hex" -testplusarg "OUT=$d/dut_out_hybrid.hex" \
